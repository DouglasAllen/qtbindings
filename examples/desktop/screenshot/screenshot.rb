...
class Screenshot < Qt::Widget
  ...
  def newScreenshot
    hide if @hideThisWindowCheckBox.checked?
  ...
  def saveScreenshot
    ...
    @originalPixmap.save(fileName, format) unless fileName.nil?
  end

  def shootScreen
    $qApp.beep if @delaySpinBox.value != 0
    ...
    show if @hideThisWindowCheckBox.checked?
  end
